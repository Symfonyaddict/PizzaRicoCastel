 ...
 * - login_attempts : 5 requêtes / minute / IP
...
final class LoginAttemptsSubscriber implements EventSubscriberInterface
{
    public function __construct(
        ...
        private readonly \Symfony\Component\RateLimiter\RateLimiterFactory $loginAttemptsLimiter,
    ) {
    }
    ...
    public function onLoginFailure(LoginFailureEvent $event): void
    {
        ...
        if ($session instanceof FlashBagAwareSessionInterface) {
            $session->getFlashBag()->add(
                'error',
                'Trop de tentatives de connexion ou identifiants invalides. Réessayez dans une minute.'
            );
        }
